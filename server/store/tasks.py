...
def generate_order_report_csv():
    ...
    total_price_today = today_orders.aggregate(Sum('total_price'))[
    ...
    total_price_month = month_orders.aggregate(Sum('total_price'))[
        'total_price__sum'] or 0
    top_selling_products_month = OrderItem.objects.filter(order__created_at__date__range=[month_ago, today]).values(
    ...
    try:
        ...
        print(f"Permission denied when trying to write to {
              filename}. Please check the file's permissions or try running the task with elevated permissions.")
